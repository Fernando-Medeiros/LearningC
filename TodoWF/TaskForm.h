...
namespace TodoWF {
	...
	public:
		User^ user;
		Task^* tasks{};
		TaskForm(void) { InitializeComponent(); }

	protected:
		~TaskForm() { if (components) { delete components; } }
...
#pragma region Windows Form Designer generated code
		   void InitializeComponent(void)
		   {
			   ...
			   this->AppendTodoBtn->Click += gcnew System::EventHandler(this, &TaskForm::AppendCommand);
			   // 
			   // PriorityCheckBox
			   // 
...
#pragma endregion
	...
	private: System::Void AppendCommand(System::Object^ sender, System::EventArgs^ e) {
		String^ query = "INSERT INTO @table VALUES ('@title', '@description')";
		query = query->Replace("@table", "tasks");
		query = query->Replace("@title", this->TitleEntry->Text);
		query = query->Replace("@description", this->DescriptionEntry->Text);
